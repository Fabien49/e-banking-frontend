...
export class ReservationService {
  ...
  protected convertDateFromClient(reservation: IReservation): IReservation {
    return Object.assign({}, reservation, {
      dateEmprunt: reservation.dateEmprunt?.isValid() ? reservation.dateEmprunt.toJSON() : undefined,
      dateRetour: reservation.dateRetour?.isValid() ? reservation.dateRetour.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateEmprunt = res.body.dateEmprunt ? dayjs(res.body.dateEmprunt) : undefined;
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((reservation: IReservation) => {
        reservation.dateEmprunt = reservation.dateEmprunt ? dayjs(reservation.dateEmprunt) : undefined;
        reservation.dateRetour = reservation.dateRetour ? dayjs(reservation.dateRetour) : undefined;
      });
    }
    return res;
  }
